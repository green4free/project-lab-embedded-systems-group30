...
with open('data2_18_30p') as csv_data:
    i = 0
    csv_reader = csv.reader(csv_data, delimiter=',')
    for row in csv_reader:
        if i < 30:
            downStrain.append(-100.0 * float(row[2]))
            downRes.append(float(row[1]))
        else:
            upStrain.append(-100.0 * float(row[2]))
            upRes.append(float(row[1]))
        i += 1
